...
router.get('/slug/:category/:product?', async (req, res) => {
  try {
      ...
      if (product) {
          const foundProduct = await productModel.findOne({ slug: product, category: foundCategory._id }).populate('category');
          if (!foundProduct) {
      ...
      const products = await productModel.find({ category: foundCategory._id });
